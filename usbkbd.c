...
struct usb_kbd {
    ...
    // data received in the past from the @irq URB representing which keys were pressed.
    unsigned char old[8];
    // irq: URB for receiving a list of keys that are pressed when a new key is pressed
    struct urb *irq;
    // URB for sending LEDs
    struct urb *led;
    // data associated with led URB
    unsigned char newleds;
    char name[128];
    char phys[64];
    // data associated with irq URB
    unsigned char *new;
    ...
    dma_addr_t new_dma;
    // DMA address for leds urb
    dma_addr_t leds_dma;
    // spinlock that protects @leds, @newleds, and @led_urb_submitted
    spinlock_t leds_lock;
    // indicates whether @led is in progress
    bool led_urb_submitted;
    // represents two different modes for CAPSLOCK
    int mode;
};
...
static int usb_kbd_event(struct input_dev *dev, unsigned int type, unsigned int code, int value){
    ...
    if(kbd->newleds == 0x01 && kbd->mode == 1) {
        printk(KERN_ALERT "From mode 1 to mode 2");
        kbd->newleds = 0x03;
        kbd->mode = 2;
    } else if(kbd->newleds == 0x00 && kbd->mode == 2) {
        printk(KERN_ALERT "From mode 2 to mode 1");
        kbd->newleds = 0x00; 
        kbd->mode = 1;
    } else if(kbd->newleds == 0x03 && kbd->mode == 2) {
        printk(KERN_ALERT "In mode 2");
        kbd->newleds = 0x01;
